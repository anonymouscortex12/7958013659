...
class Neuron {
	...
	private:
	unsigned int * uid; // unique global identifier of the neuron
	...
	public:
	Neuron();
	Neuron(unsigned int * unique_id, float * params, float * neiWeights, unsigned int * neighbors, uint32_t numNeighbors);
	void setValues(unsigned int * unique_id, float * params, float * neiWeights, unsigned int * neighbors, uint32_t numNeighbors);
